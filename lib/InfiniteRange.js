...
export const numericValueFromEvent = ({ target: { value } }) => parseInt(value, 10);
...
export class InfiniteRange extends Component {
  ...
  externalUpdate = value => {
    this.setState(InfiniteRange.initialState({ ...this.props, defaultValue: value }));
  };
          ...
          {interpolatedStyle => {
            ...
            if (this.isChanging === false) {
              ...
              inputProps = {
                ...
                min: snapIfWithinOffset(interpolatedRoundedStyle.min, this.state.min, 10),
                max: snapIfWithinOffset(interpolatedRoundedStyle.max , this.state.max, 10)
              };
            }
...
const snapIfWithinOffset = (current, target, offset) => {
  return Math.abs(target - current) > offset
    ? current
    : target;
};
