...
describe('initialState', () => {
  it('should reset value to floor if it is smaller', () => {
    expect(
      InfiniteRange.initialState({ chunkSize: 500000, floor: 100, defaultValue: 0 })
    ).toEqual({ value: 100, max: 500000, min: 100 });
  });

  it('should reset value to ceiling if it is larger', () => {
    expect(
      InfiniteRange.initialState({ ceiling: 1000000000, defaultValue: 1000000000 + 1 })
    ).toEqual({ value: 1000000000 });
  });

  it('should produce the initial state with min, max and value', () => {
    expect(
      InfiniteRange.initialState({ defaultValue: 100, floor: 100, chunkSize: 500000 })
    ).toEqual({ max: 500000, min: 100, value: 100 });
  });

  it('should produce correct state if value is smaller than `chunkSize`', () => {
    expect(
      InfiniteRange.initialState({ defaultValue: 25000, floor: 100, chunkSize: 500000 })
    ).toEqual({ max: 500000, min: 100, value: 25000 });
  });

  it('should produce correct state if value is larger than `chunkSize`', () => {
    expect(
      InfiniteRange.initialState({ defaultValue: 500100, floor: 100, chunkSize: 500000 })
    ).toEqual({ max: 1000100, min: 100, value: 500100 });
  });
});
